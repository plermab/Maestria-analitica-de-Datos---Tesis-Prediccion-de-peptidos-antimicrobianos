...
def sample(
    ...
    # prompt is a tensor of token ids, with shape (B, T), in case of bidi smapling
        ...
        x = prompt.to(device).to(torch.int64)                        # (B, T)
    else:
    ...
    # 🔹 Elegir una longitud objetivo aleatoria entre min_length y max_length
    
    target_length = torch.randint(min_length, max_length + 1, (1,)).item()

    pbar = tqdm(total=target_length - generated.shape[-1])
...
def truncate(seq: str , min_length: int) -> str:
    ...
    #  Si quedó vacío después de truncar, devolvemos lo que había antes
...
def main(args):
    ...
    os.environ["TRANSFORMERS_NO_ADVISORY_WARNINGS"] = "1"  # evita prompt
    ...
    if str(device) == "cpu" and args.batch_size > 1:
        logger.warning(
            f"Estás usando CPU con batch_size={args.batch_size}, la inferencia será lenta. "
            "Considera usar GPU o un batch más pequeño."
        )
    ...
    if os.path.exists(os.path.join(args.model, "tokenizer.json")):
        tokenizer: Tokenizer = Tokenizer.from_file(
            os.path.join(args.model, "tokenizer.json")
        )
    else:
    ...
    logger.debug(f"Tokenizer vocab size: {tokenizer.vocab_size}")
    ...
    output_file = os.path.join(
        samples_dir,
        f"samples_ctx{args.prompt}_k{args.k}_t{args.t}_min{args.min_length}_max{args.max_length}.fa",
    )

    # Ajustar top-k según vocabulario
    if args.k == 0 or args.k > model.config.vocab_size:
        args.k = None

    logger.debug(
        f"Sampling parameters: top_k={args.k}, temperature={args.t}, "
        f"min_length={args.min_length}, max_length={args.max_length}"
    )

    tokens = tokenizer.tokenize(args.prompt)
    logger.info(f"Prompt tokens: {tokens}")

    # Ajustar longitud si es bidireccional
    if args.bidirectional:
        args.max_length = (args.max_length - len(tokens)) // 2
        if len(tokens) <= 2:
            logger.warning(
                "Prompt demasiado corto para bidirectional sampling. Usa un prompt más largo."
            )
        ...
        for i in range(args.iters):
            ...
            if args.bidirectional:
                ...
                for rs in reversed_samples:
                    prompt = torch.tensor(tokenizer.encode(rs)).view(1, -1).to(model.device)
                    samples.extend(
                        sample(
                            model=model,
                            tokenizer=tokenizer,
            ...
            for j, c in enumerate(samples):
                    # intentar hasta 3 veces generar una secuencia válida
                    attempt = 0
                    valid_seq = ""
                    while attempt < 3 and not valid_seq:
                        seq = truncate(c, min_length=args.min_length)
                        if seq and len(seq) >= args.min_length:
                            valid_seq = seq
                        else:
            # volver a muestrear una nueva secuencia
                            new_sample = sample(
                                model=model,
                                tokenizer=tokenizer,
                                device=device,
                    ...
                    if not valid_seq:
                        continue  # si tras 3 intentos no hay nada válido, saltar
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum length of the generated sequence. Default: 1",
    )
    parser.add_argument(
        "--max_length",
        type=int,
